...
{
    /// <summary>
    /// 负责场景中游戏对象的加载，对象全部来源于Resources，可使用缓冲池技术
    /// 智能清理：改进为若缓冲池的GO还没有active=false时创建新GameObject，并在一定时间后销毁
    /// 对于特殊的GO，可以不使用这里提供的缓冲池
    /// </summary>
    public class ResourcesMgr : IGameMgr
    {
        ...
        /// <returns></returns>
        public T LoadResource<T>(string path, bool isCatch) where T : Object
        {
            if (ht.Contains(path))
            {
                Debug.Log("Contain");
